...
def get_bandwidth_usage():
    """Get total bytes sent and received on all network interfaces."""
    net_io = psutil.net_io_counters()
    return net_io.bytes_sent, net_io.bytes_recv

def format_bandwidth(bytes):
    """Convert bytes to a more readable format (KB, MB, GB, TB)."""
    if bytes < 1024:
...
@FileStream.on_message(filters.command("uses") & filters.private & filters.user(Telegram.OWNER_ID))
async def get_uses(c: Client, m: Message):
    ...
    total_bandwidth = format_bandwidth(bytes_sent + bytes_recv)

    # Prepare and send the message
    await m.reply_text(
        f"**Bandwidth Usage**\n"
        f"Total Sent: {formatted_sent}\n"
        f"Total Received: {formatted_recv}\n"
        f"Total Bandwidth Used: {total_bandwidth}"
    )
